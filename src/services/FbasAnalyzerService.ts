...
export default class FbasAnalyzerService {
    ...
    //todo: move to domain
    async performAnalysis(network: Network): Promise<Result<AnalysisResult, Error>> {
        try {
            let faultyNodes = JSON.stringify(network.nodes
                .filter(node => network.isNodeFailing(node))
                .map(node => node.publicKey));

            let correctlyConfiguredNodes = JSON.stringify(network.nodes.filter(node => node.isValidator && this.isNodeCorrectlyConfigured(node)));
            ...
            let symmetricTopTierAnalysis: SymmetricTopTierAnalysis = stellar_analysis.analyze_symmetric_top_tier(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.DoNotMerge
            );

            let topTierOrganizationAnalysis: TopTierAnalysis = stellar_analysis.analyze_top_tier(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.Orgs
            );

            let blockingSetsAnalysis: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                JSON.stringify([]),
                stellar_analysis.MergeBy.DoNotMerge
            ...
            let blockingSetsAnalysisFiltered: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                faultyNodes,
                stellar_analysis.MergeBy.DoNotMerge
            ...
            let orgBlockingSetsAnalysis: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                JSON.stringify([]),
                stellar_analysis.MergeBy.Orgs
            ...
            let orgBlockingSetsAnalysisFiltered: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                faultyNodes,
                stellar_analysis.MergeBy.Orgs
            ...
            let countryBlockingSetsAnalysis: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                JSON.stringify([]),
            ...
            let countryBlockingSetsAnalysisFiltered: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                faultyNodes,
                stellar_analysis.MergeBy.Countries
            ...
            let ispBlockingSetsAnalysis: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                JSON.stringify([]),
            ...
            let ispBlockingSetsAnalysisFiltered: BlockingSetsAnalysis = stellar_analysis.analyze_minimal_blocking_sets(
                correctlyConfiguredNodes,
                organizations,
                faultyNodes,
                stellar_analysis.MergeBy.ISPs
            ...
            let splittingSetsAnalysis: SplittingSetsAnalysis = stellar_analysis.analyze_minimal_splitting_sets(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.DoNotMerge
            );

            let orgSplittingSetsAnalysis: SplittingSetsAnalysis = stellar_analysis.analyze_minimal_splitting_sets(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.Orgs
            );

            let countrySplittingSetsAnalysis: SplittingSetsAnalysis = stellar_analysis.analyze_minimal_splitting_sets(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.Countries
            );

            let ispSplittingSetsAnalysis: SplittingSetsAnalysis = stellar_analysis.analyze_minimal_splitting_sets(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.ISPs
            );

            let minimalQuorums: MinimalQuorumsAnalysis = stellar_analysis.analyze_minimal_quorums(
                correctlyConfiguredNodes,
                organizations,
                stellar_analysis.MergeBy.DoNotMerge
            );
            ...
            return ok({
                ...
                minimalBlockingSetsFaultyNodesFiltered: blockingSetsAnalysisFiltered.result,
                minimalBlockingSetsFaultyNodesFilteredMinSize: blockingSetsAnalysisFiltered.min,
                orgMinimalBlockingSets: orgBlockingSetsAnalysis.result,
                orgMinimalBlockingSetsMinSize: orgBlockingSetsAnalysis.min,
                orgMinimalBlockingSetsFaultyNodesFiltered: orgBlockingSetsAnalysisFiltered.result,
                orgMinimalBlockingSetsFaultyNodesFilteredMinSize: orgBlockingSetsAnalysisFiltered.min,
                ...
                countryMinimalBlockingSetsFaultyNodesFiltered: countryBlockingSetsAnalysisFiltered.result,
                countryMinimalBlockingSetsFaultyNodesFilteredMinSize: countryBlockingSetsAnalysisFiltered.min,
                ispMinimalBlockingSets: ispBlockingSetsAnalysis.result,
                ispMinimalBlockingSetsMinSize: ispBlockingSetsAnalysis.min,
                ispMinimalBlockingSetsFaultyNodesFiltered: ispBlockingSetsAnalysisFiltered.result,
                ispMinimalBlockingSetsFaultyNodesFilteredMinSize: ispBlockingSetsAnalysisFiltered.min,
