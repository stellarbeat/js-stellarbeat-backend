...
it('should find latest scan', async function () {
	...
	await repo.save([
		HistoryArchiveScan.create(olderDate, createDummyHistoryBaseUrl(), 0, 1000),
		HistoryArchiveScan.create(latestDate, createDummyHistoryBaseUrl(), 0, 1000)
	]);
	...
	expect(latest.scanDate.getTime()).toEqual(latestDate.getTime());
});
